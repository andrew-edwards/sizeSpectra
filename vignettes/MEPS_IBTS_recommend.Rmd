...
for(i in 1:length(fullYears))
  {
    ...
    rm(data.year)      # TODO not good style but think needed for next section
}
...
{
  data.year = data.year.list[[i]]


  ISD_bin_plot(TODO) INPUTS:
                       data.year
  year = fullYears[i],
  b.MLE = dplyr::filter(MLEbins.res, Year == fullYears[i])$b,
  confMin = dplyr::filter(MLEbins.res, Year == fullYears[i])$confMin,
  confMax = dplyr::filter(MLEbins.res, Year == fullYears[i])$confMax,
