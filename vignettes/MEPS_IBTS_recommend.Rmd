...
\subsection{Checking cumSum -- 1991 ($i=6$) didn't agree with countGTEwmin calculation;
  see later for automated calculations for each year this occurs}

For $i=6$, year 1991, we get {\tt cumSum} and {\tt countGTEwmin} differing by
...
use {\tt countGTEwmin} to plot the horizontal values, not {\tt cumSum}. Not that you'd
...
\subsection{Check cumSum -- 1992 ($i=7$) didn't agree with countGTEwmin calculation}

For $i=7$, year 1992, we get {\tt cumSum} and {\tt countGTEwmin} differing in
...
data.diff = dplyr::filter(data.one.year, abs(cumSum - countGTEwmin) > 0)
...
{\tt countGTEwmin}, and give the maximum difference at the end. Take a look here
...
understanding).
<<year.others>>=
max.abs.diff = numeric()        # maximum overall abs diff betw cumSum and countGTEwmin
for(diff.i in diff.ivec)
  {
    ...
    data.one.year = data.year.list[[diff.i]]
    data.diff = dplyr::filter(data.one.year, abs(cumSum - countGTEwmin) > 0)
    print(data.diff$cumSum - data.diff$countGTEwmin)
    max.abs.diff = max(max.abs.diff,
                       max(abs(data.diff$cumSum - data.diff$countGTEwmin)))
    dataBin.diff = dplyr::filter(dataBin, Year == diff.year, wmin %in% data.diff$wmin)
    # If it's again because two species have same length-weight coefficients then
    #  should be able to manually spot that by arranging the rows:
...
{\tt cumSum} and {\tt countGTEwmin} are due to multiple species having the same
...
it's actually {\tt countGTEwmin} that is the lower value. This only affects the
plotting of the y-values of the segments, not the rectangles at all (and so
at the end of the day is not that important), but we use {\tt countGTEwmin} in the
plots anyway.
