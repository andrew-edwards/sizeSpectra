...
## Setting $x_{min}=16$ (Figures S.35 and S.36 and Table S.4)

Similarly, the simulation results obtained from setting $x_{min}=16$ are saved in
...
Note that any very minor differences in results to those in the published
figures and tables are due to the original code that produced the published
versions using up a random number (after I set the seed to 42) when doing
`require(dplyr)`. Such differences are not important.
