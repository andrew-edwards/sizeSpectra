...
for(i in 1:MLEbin.results$MLE.array.parameters$binTypes)
  {
    MLEbin.table[2*i - 1, ] =
      c(MLEbin.results$MLE.array.parameters$binType.name[[i]],
        names(MLEbin.MEPS.default$MLE.array[1,1,])[1],
        qqtab(MLEbin.MEPS.default$MLE.array[ ,i,1],
              quants=c(0.05, 0.95),
              type="data.frame",
              true = MLEbin.results$MLE.array.parameters$b.known)
        )
HERE do a loop for j = 1,2   (number of methods)     each row is 2*i - 1 + j-1 =
                                                                   2*i + j - 3
    ...
    HERE
        MLEbin.results$MLE.array.parameters$binType.name

    print(paste(binType.name[[i]],
                " | ",
                MLEmethod.name[1],
                " | ",
                qqtab(MLE.array[ ,i,1], quants=c(0.05, 0.95), latex=FALSE)),
          quote=FALSE)
    print(paste(" | ", MLEmethod.name[2], " | ",
                qqtab(MLE.array[ ,i,2], quants=c(0.05, 0.95), latex=FALSE)),
          quote=FALSE)
}
```
