...
Data extracted from IBTS DATRAS website TODO: copy details from paper and put
some of Julia's earlier code in; may want to use the even raw-er data; just
getting this working for now. This may
...
al. (2005), since data are unreliable for smaller organisms [TODO: look at
sensitivity if time].
```{r}
...
```{r halfcm}
temp = dplyr::filter(IBTS.data, !(SpecCode %in% c(126417, 126425)))
...
These next commands here (which are not run in this vignette) are to save `IBTS.data`
...
of trawling observed for the combination. Parameters `LWa` and `LWb` [TODO:
change to alpha, beta] are the length-weight cofficients for that species from
Fung et al. (2012), `bodyMass` (g) is the resulting estimated body mass for an
individual of that species and length class.

The resulting Table 1 is (more or less): [TODO: need last column]

```{r results='asis'}
knitr::kable(rbind(IBTS.data[1:5,],
                   IBTS.data[ (nrow(IBTS.data)-5):nrow(IBTS.data), ]),
             digits=c(0, 0, 0, 3, 4, 4, 2))
```
