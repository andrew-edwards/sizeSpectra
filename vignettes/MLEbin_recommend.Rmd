...
regression (which would be the LBNbiom method). The LBNbiom method for fitting
the red lines would weight
all bins equally, so the few individuals with large body size are given equal
weight to the numerous small individuals.



XXX - A useful thing will be to create movie where the fit is for incrementally higher
values of $x_{min}$
...
## Extra - note I already have a total biomass function - should check it agrees
## with pBiomassbins


Some rough calculations:
```{r biomass, eval = FALSE}
bio <- pBiomass(b = MLEbin.res$MLE,
         xmin = min(x.binned$binVals$binMin),
         xmax = max(x.binned$binVals$binMax),
         n = 1000,
         x1 = 256,   # just testing hardwired values
         x2 = 512)

log10(bio/256)

bioConfMin <- pBiomass(b = MLEbin.res$conf[1],
         xmin = min(x.binned$binVals$binMin),
         xmax = max(x.binned$binVals$binMax),
         n = 1000,
         x1 = 256,   # just testing hardwired values
...
log10(bioConfMin/256)

bioConfMax <- pBiomass(b = MLEbin.res$conf[2],
         xmin = min(x.binned$binVals$binMin),
         xmax = max(x.binned$binVals$binMax),
         n = 1000,
         x1 = 256,   # just testing hardwired values
...
log10(bioConfMax/256)

```

Then put these calcs into the plotting function as an option - blue box maybe.
