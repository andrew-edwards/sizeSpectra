...
gives Figures 6, S.1, S.2 and S.3.
...
dataBinSpec = dplyr::mutate(dataBinSpec, SpecCode = factor(SpecCode, levels =
...
dataBinSpec = dplyr::mutate(dataBinSpec, wWidth = wmax - wmin,
...
dataBinMaxRatio = dplyr::filter(dataBin, SpecCode == specCodeMaxRatio,
...
for(fig.num in 1:4)      # doing 3 figures
    ...
    yLim = 1.02*max(dplyr::filter(dataBinSpec, SpecCode %in% specForFig)$wmax)
    ...
    for(ii in 1:length(specForFig))   # loop over species, plot bins for each
        ...
        segments(x0 = xVal, y0 = dplyr::filter(dataBinSpec,
                                SpecCode == specForFig[ii])$wmin,
         y1 = dplyr::filter(dataBinSpec, SpecCode == specForFig[ii])$wmax,
